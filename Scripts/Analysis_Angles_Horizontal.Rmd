---
title: "LiveLink Experiment, Horizontal Angles"
...
The goal of this markdown file is to perform the analysis for the Horizontal 
...
disregarded. Although the values of these columns are different, in this case 
they simply mean that it concerns a different recording. 

```{r Load data}
data <- read.csv('Data/Data_Horizontal.csv', stringsAsFactors=TRUE)
```
...
We divide the main dataframe into two separate dataframes: one for the horizontal
angle going right, and one for the horizontal angle going left. We check the 
levels for Camera to ensure that data_horizontal_going_right only contains C0 
and C3; and data_horizontal_going_left only contains C0 and C4.

```{r Separate}
data_horizontal_going_right <- data[data$Camera != "C4",]
data_horizontal_going_right <- droplevels(data_horizontal_going_right)
levels (data_horizontal_going_right$Camera)

data_horizontal_going_left <- data[data$Camera != "C3",]
data_horizontal_going_left <- droplevels(data_horizontal_going_left)
levels (data_horizontal_going_left$Camera)
```
...
at -1/2; and C3 and C4 are the comparison cameras coded at +1/2. We check that
the contrasts are applied to the dataframes correctly.

```{r Add contrasts}
camera.contrast <- cbind (c(-0.5, +0.5))   # -C0+C3, -C0+C4
colnames (camera.contrast) <- c("-C0+C3")
contrasts (data_horizontal_going_right$Camera) <- camera.contrast
contrasts (data_horizontal_going_right$Camera)

colnames (camera.contrast) <- c("-C0+C4")
contrasts (data_horizontal_going_left$Camera) <- camera.contrast
contrasts (data_horizontal_going_left$Camera)
```
...
### Camera 0 vs Camera 3: Horizontal Angle Going Right

```{r C0 vs C3, message=FALSE}
# Create empty results dataframes
results_3HA <- data.frame(Blendshape = character(),
...
results_3LA <- data.frame(Blendshape = character(),
                      Activation = character(),
                      Intercept = numeric(),
                      Effect = numeric(),
                      Perc_eff = numeric(),
...
data_horizontal_going_rightC <- data_horizontal_going_right
for(blendshape in blendshapes) {
  data_horizontal_going_rightC <- classify(data_horizontal_going_rightC, blendshape)
}

# Fit the model and update the results data frame for each blendshape
for (blendshape in blendshapes) {
  results_3HA <- fit_model(data_horizontal_going_rightC, blendshape, results_3HA, 
                           "3", "HA")
  results_3LA <- fit_model(data_horizontal_going_rightC, blendshape, results_3LA, 
                           "3", "LA")
}

# Update row names for consecutive numbering
rownames(results_3HA) <- 1:nrow(results_3HA) 
rownames(results_3LA) <- 1:nrow(results_3LA) 
```


### Camera 0 vs Camera 4: Horizontal Angle Going Left

```{r C0 vs C4, message=FALSE}
# Create empty results dataframes
results_4HA <- data.frame(Blendshape = character(),
                      Activation = character(),
                      Intercept = numeric(),
                      Effect = numeric(),
                      Perc_eff = numeric(),
...
results_4LA <- data.frame(Blendshape = character(),
                      Activation = character(),
                      Intercept = numeric(),
                      Effect = numeric(),
                      Perc_eff = numeric(),
...
# Classify the frames for each blendshape
data_horizontal_going_leftC <- data_horizontal_going_left
for(blendshape in blendshapes) {
  data_horizontal_going_leftC <- classify(data_horizontal_going_leftC, blendshape)
}

# Fit the model and update the results data frame for each blendshape
for (blendshape in blendshapes) {
  results_4HA <- fit_model(data_horizontal_going_leftC, blendshape, results_4HA, 
                           "4", "HA")
  results_4LA <- fit_model(data_horizontal_going_leftC, blendshape, results_4LA, 
                           "4", "LA")
}

# Update row names for consecutive numbering
rownames(results_4HA) <- 1:nrow(results_4HA)
rownames(results_4LA) <- 1:nrow(results_4LA)
```
...
```{r Save results}
write.csv(results_3HA, "../Results/Results_Horizontal_Right_HA.csv", row.names=FALSE)
write.csv(results_3LA, "../Results/Results_Horizontal_Right_LA.csv", row.names=FALSE)
write.csv(results_4HA, "../Results/Results_Horizontal_Left_HA.csv", row.names=FALSE)
write.csv(results_4LA, "../Results/Results_Horizontal_Left_LA.csv", row.names=FALSE)

write.csv(data_horizontal_going_rightC, "Data/Data_Horizontal_Right_Classified.csv", row.names=FALSE)
write.csv(data_horizontal_going_leftC, "Data/Data_Horizontal_Left_Classified.csv", row.names=FALSE)
```
...
tmp <- data_horizontal_going_right[data_horizontal_going_right$ID == idx,]
...
line_colors <- c("C0" = "darkslategrey", "C3" = "chocolate")
...
ggplot(tmp, aes(x = FrameNr, y = !!sym(blendshape), color = Camera)) +
  ...
  scale_color_manual(values = line_colors, labels = c("C0", "C3")) + 
...
for(blendshape in blendshapes) {
  for(idx in levels(data_horizontal_going_leftC$ID)) {
    name <- paste("plots/", blendshape, "_", idx, ".png", sep="")
    tmp <- data_horizontal_going_leftC[data_horizontal_going_leftC$ID == idx,]
    tmp <- droplevels(tmp)
    color_df <- create_color_df(tmp[tmp$Camera == "C0",], blendshape)
